...
async function loadReportsData() {
  ...
  const costsData = expenseResults.map(r =>
    (r.data || []).reduce((s, x) => s + Number(x.amount), 0)
  );
  const incomesData = incomeResults.map(r =>
    (r.data || []).reduce((s, x) => s + Number(x.amount), 0)
  );
...
function renderCharts(labels, costsData, incomesData, savingsData) {
  ...
  const configs = [
    {
      id: "chartIncomes",
      data: incomesData,
      colorSolid: "rgba(118, 255, 176, 1)",
      colorTop: "rgba(118, 255, 176, 0.55)",
      colorBottom: "rgba(118, 255, 176, 0.05)",
    },
    {
      id: "chartCosts",
      data: costsData,
      colorSolid: "rgba(255, 142, 183, 1)",
      colorTop: "rgba(255, 142, 183, 0.55)",
      colorBottom: "rgba(255, 142, 183, 0.05)",
    },
  ];
  ...
  configs.forEach(cfg => {
    ...
    chartInstances[cfg.id] = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [{
          data: cfg.data,
          backgroundColor: makeGradient(ctx, cfg.colorTop, cfg.colorBottom),
          borderColor: cfg.colorSolid,
          borderWidth: 2,
          borderRadius: 8,
          borderSkipped: false,
        }]
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: { label: c => `  ${c.parsed.y.toFixed(2)} zł` }
          }
        },
        scales: {
  ...
  // Savings chart — per-bar color based on positive/negative
  if (chartInstances.chartSavings) chartInstances.chartSavings.destroy();
  const savingsCanvas = document.getElementById("chartSavings");
    ...
    data: {
      ...
      datasets: [{
        ...
        backgroundColor: savingsData.map(v =>
          v >= 0 ? "rgba(118, 255, 176, 0.45)" : "rgba(255, 142, 183, 0.45)"
        ),
        borderColor: savingsData.map(v =>
          v >= 0 ? "rgba(118, 255, 176, 1)" : "rgba(255, 142, 183, 1)"
        ),
        borderWidth: 2,
        borderRadius: 8,
        borderSkipped: false,
